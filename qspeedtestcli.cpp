...
void QSpeedTestCli::printHostAndProgramInfo()
{
    ...
    updateTestResults(trUtf8("Test date and time:       %1\n"
                             "Host OS & no. of CPUs:    %2 / %3").arg(results.testDateTime).arg(results.hostOS).arg(results.cpuCores));
}


void QSpeedTestCli::getLineInfo()
{
    if(download)
    {
        download->abort();
...
void QSpeedTestCli::printLineInfo()
{
    ...
    delete download;
    download = NULL;
    updateTestResults(trUtf8("ISP & WAN IP:             %1 - %2\n"
...
void QSpeedTestCli::startBenchmark()
{
    ...
    updateTestResults(trUtf8("Test mode:                %1\n"
    ...
    if(pingTestEnabled)
    {
        ...
        updateTestResults(trUtf8(""
                             "Targets alive:            %1 / %2\n"
                             "Average ping/target:      %3").arg(results.targetsAlive).arg(results.targetsTotal).arg(results.getRttAvgAsString()));
    }

    if(downloadTestEnabled)
    {
        updateTestResults(trUtf8(""
                             "Speed test domestic:      %1 Kbps\n"
                             "                          %2 MB/sec").arg(results.speedInKbpsDomestic, 0, 'f', 0).arg(results.speedInMBpsDomestic, 0, 'f', 3));
        updateTestResults(trUtf8(""
                             "Speed test international: %1 Kbps\n"
